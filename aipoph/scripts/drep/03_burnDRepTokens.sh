...
total_drep_lock_value=$((${drep_lock_value} - ${burn_amt}))

negative_burn_amt=$((-1 * ${burn_amt}))
# update the add_amt
variable=${negative_burn_amt}; jq --argjson variable "$variable" '.fields[0].int=$variable' ../data/drep/burn-redeemer.json > ../data/drep/burn-redeemer-new.json
mv ../data/drep/burn-redeemer-new.json ../data/drep/burn-redeemer.json

#!/bin/bash

# Your numeric variable
if [ "$total_drep_lock_value" -eq 0 ]; then
    script_address_out="${drep_lock_script_address} + ${drep_lock_lovelace_value}"
else
...
burn_tokens="${negative_burn_amt} ${policy_id}.${token_name}"
tokens="${burn_amt} ${dao_pid}.${dao_tkn}"
...
FEE=$(${cli} transaction build \
    ...
    --policy-id="${policy_id}" \
    --mint-reference-tx-in-redeemer-file ../data/drep/burn-redeemer.json \
    ${network})
